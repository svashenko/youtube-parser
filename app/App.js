...
async function App(url) {
  ...
  try {
    const raw = await youtubeDl(url, { dumpSingleJson: true, addHeader: ['referer:youtube.com', 'user-agent:googlebot'], cookiesFromBrowser: 'chrome' });
    ...
    await youtubeDl(url, { f: '136/135/134/bestvideo[ext=mp4]', o: videoPath, addHeader: ['referer:youtube.com', 'user-agent:googlebot'], cookiesFromBrowser: 'chrome' });

    audioPath = path.join(downloadsDir, 'audioOnly.m4a');
    await youtubeDl(url, { f: '140/bestaudio[ext=m4a]', o: audioPath, addHeader: ['referer:youtube.com', 'user-agent:googlebot'], cookiesFromBrowser: 'chrome' });
